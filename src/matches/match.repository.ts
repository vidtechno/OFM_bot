...
import { assignGoalParticipants } from "./event-participants.js";
...
export interface MatchOwnerReport {
  telegramId: number;
  clubId: string;
  club: string;
  opponent: string;
  ...
  homeGoals: number;
  awayGoals: number;
  goals: Array<{ minute: number; player: string; assist: string | null; club?: string }>;
  ...
  income: number;
  balance: number;
  next: { home: string; away: string; scheduledAt: string } | null;
  leagueName: string;
  leagueTable?: Array<{ position: number; club: string; played: number; goalDifference: number; points: number }>;
}
...
export class MatchRepository {
  ...
  async claimDue(limit = 20, workerId: string = crypto.randomUUID()): Promise<DueMatch[]> {
    ...
    if (error) throw error;
    try {
      return await Promise.all((data ?? []).map(async (fixture: any) => ({
        fixtureId: fixture.fixture_id,
        home: await this.team(fixture.home_club_id),
        away: await this.team(fixture.away_club_id),
      })));
    } catch (claimError) {
      await Promise.allSettled((data ?? []).map((fixture: any) => this.releaseClaim(fixture.fixture_id, workerId)));
      throw claimError;
    }
  }

  async releaseClaim(fixtureId: string, workerId: string): Promise<void> {
  ...
  private async checkSeasonCompletion(matchId: string): Promise<void> {
    ...
    // One idempotent database transaction archives every human manager's season,
    // updates lifetime career counters, records podium honours and queues summaries.
  ...
  async repairPlayerStats(matchId: string): Promise<void> {
    await this.recordPlayerStats(matchId, true);
  }

  private async recordPlayerStats(matchId: string, replaceExisting = false): Promise<void> {
    ...
    const assign = async (clubId: string, goals: number) => {
      ...
      const [{ data, error }, { data: events, error: eventError }, { data: lineup }] = await Promise.all([
        ...
        this.database
          .from("lineups")
          .select("penalty_taker_player_id, lineup_players(club_player_id)")
          .eq("league_club_id", clubId)
          .maybeSingle(),
      ]);
      ...
      const starterIds = new Set(((lineup as any)?.lineup_players ?? []).map((row: any) => row.club_player_id));
      ...
      for (const assignment of assignments) {
        ...
        const { error: updateEventError } = await this.database
          .from("match_events")
          .update({
            player_id: scorer.id,
  ...
  async ownerReports(matchId: string): Promise<MatchOwnerReport[]> {
    const { data: match, error: matchError } = await this.database
      .from("matches")
      .select("league_instance_id,home_club_id,away_club_id,home_goals,away_goals,fixtures!inner(round_number),match_stats(*)")
      .eq("id", matchId)
      .single();
    if (matchError) throw matchError;
    ...
    const [{ data: clubs, error: clubError }, { data: events, error: eventError }] = await Promise.all([
      this.database
        .from("league_clubs")
        .select("id,manager_type,manager_user_id,points,played,wins,draws,losses,cash_balance,clubs!inner(name),league_instances!inner(instance_number,competitions!inner(name)),users(telegram_id)")
        .in("id", [match.home_club_id, match.away_club_id]),
      this.database
        .from("match_events")
        .select("minute,club_id,player_id,metadata,players(short_name)")
        .eq("match_id", matchId)
        .eq("event_type", "GOAL")
        .order("minute"),
    ]);
    ...
    const assistantIds = (events ?? []).map((event: any) => event.metadata?.assist_player_id).filter(Boolean);
    const { data: assistants, error: assistError } = assistantIds.length
      ? await this.database.from("players").select("id,short_name").in("id", assistantIds)
      : { data: [], error: null };
    if (assistError) throw assistError;
    const assistantName = new Map((assistants ?? []).map((player: any) => [player.id, player.short_name]));

    const { data: allTable, error: tableError } = await this.database
      .from("league_clubs")
      .select("id,played,points,goals_for,goals_against,clubs!inner(name)")
    ...
    const leagueTable = ordered.map((row: any, index: number) => ({
      position: index + 1,
      club: first<any>(row.clubs).name,
      played: row.played,
      goalDifference: row.goals_for - row.goals_against,
      points: row.points,
    }));

    const stats = first<any>((match as any).match_stats);
    ...
    const goals = (events ?? []).map((event: any) => ({
      minute: event.minute,
      player: first<any>(event.players)?.short_name ?? "Noma’lum",
      assist: assistantName.get(event.metadata?.assist_player_id) ?? null,
      club: first<any>(clubMap.get(event.club_id)?.clubs)?.name ?? "Noma’lum klub",
    }));

    const result: MatchOwnerReport[] = [];
    for (const club of [home, away]) {
      if (club.manager_type !== "HUMAN") continue;
      const user = first<any>(club.users);
      if (!user?.telegram_id) continue;
      const isHome = club.id === match.home_club_id;
      const opponent = isHome ? away : home;
      const { data: incomeRows, error: incomeError } = await this.database
        .from("finance_transactions")
        .select("amount")
        .eq("match_id", matchId)
        .eq("league_club_id", club.id);
      if (incomeError) throw incomeError;
      const { data: next, error: nextError } = await this.database
        .from("fixtures")
        .select("scheduled_at,home:league_clubs!fixtures_home_club_id_fkey(clubs!inner(name)),away:league_clubs!fixtures_away_club_id_fkey(clubs!inner(name))")
      ...
      const league = first<any>(club.league_instances);
      const competition = first<any>(league.competitions);

      result.push({
        telegramId: Number(user.telegram_id),
        clubId: club.id,
        club: first<any>(club.clubs).name,
        opponent: first<any>(opponent.clubs).name,
        isHome,
        ...
        position: ordered.findIndex((row: any) => row.id === club.id) + 1,
        points: club.points,
        played: club.played,
        wins: club.wins,
        draws: club.draws,
        losses: club.losses,
        income: (incomeRows ?? []).reduce((sum: number, row: any) => sum + Number(row.amount), 0),
        balance: Number(club.cash_balance),
        next: next ? {
          home: first<any>(first<any>((next as any).home).clubs).name,
          away: first<any>(first<any>((next as any).away).clubs).name,
