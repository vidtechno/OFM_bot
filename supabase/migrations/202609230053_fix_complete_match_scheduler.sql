...
-- Keep match completion idempotent and compatible with the scheduler schema.
-- This migration intentionally contains no pg_cron/pg_net statements.
...
begin
  ...
  insert into public.match_events(match_id,minute,event_type,club_id,is_penalty,metadata)
  select v_match,(e->>'minute')::smallint,e->>'type',case when e->>'side'='HOME' then v_fixture.home_club_id else v_fixture.away_club_id end,
    coalesce((e->>'isPenalty')::boolean,false),'{}'::jsonb
